...
async function postRequestReviewSingle(req, res) {
  ...
  try {
    ...
    if (result.ok) {
      ...
      const jours = skeepers.delaiSollicitation();
      const quand = skeepers.dateEnvoiPrevue();
      /* AVEC L'HEURE quand il y a une attente, pas seulement le jour : annoncer
         « jeudi » sur une échéance à 19h22 laisse croire à un envoi imminent
         alors qu'il reste treize heures. C'est ce flou qui a fait conclure à
         une panne. À délai 0, il n'y a rien à programmer : Skeepers envoie dès
         qu'il traite l'événement, et donner une heure précise serait inventer
         une garantie qu'on n'a pas. */
      const quandTxt = jours === 0
        ? 'sans délai (Skeepers envoie dès le traitement de l\'événement)'
      ...
      return res.json({
        ...
        message: 'Acceptée par Skeepers. Envoi ' + quandTxt
          + (u && u.email ? ' à ' + u.email : '') + '.',
