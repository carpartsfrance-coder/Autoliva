...
/** L'échappement regex réel du projet, en version locale (pur, sans base). */
...
test('le repli ne rapatrie plus tout le catalogue', async (t) => {
  await t.test('il ne garde que les fiches contenant un des mots cherchés', () => {
    const f = svc.filtreTexteRepli({ archived: false }, 'mecatronique dq200', esc);
    ...
    /* Chaque mot est cherché sur chacun des champs classés. */
    assert.ok(ou.some((c) => c.name && /mecatronique/.test(c.name.$regex)));
    assert.ok(ou.some((c) => c.sku && /dq200/.test(c.sku.$regex)));
  });
  ...
  await t.test('un filtre $or existant n’est pas écrasé', () => {
    ...
    const f = svc.filtreTexteRepli(origine, 'boite', esc);
  ...
  await t.test('les mots d’un seul caractère sont écartés', () => {
    ...
    assert.deepEqual(svc.filtreTexteRepli({ x: 1 }, 'a', esc), { x: 1 });
    assert.deepEqual(svc.filtreTexteRepli({ x: 1 }, 'a b c', esc), { x: 1 });
    assert.deepEqual(svc.filtreTexteRepli({ x: 1 }, '', esc), { x: 1 });
    assert.deepEqual(svc.filtreTexteRepli({ x: 1 }, '   ', esc), { x: 1 });
  });
  ...
  await t.test('aucun métacaractère regex ne peut atteindre la requête', () => {
    ...
    assert.deepEqual(svc.filtreTexteRepli({}, '.*', esc), {}, 'aucun mot exploitable');
    assert.deepEqual(svc.filtreTexteRepli({}, '.*.*.*', esc), {});
    assert.deepEqual(svc.filtreTexteRepli({}, '(((', esc), {});

    const f = svc.filtreTexteRepli({}, 'dq(200)|.*', esc);
    ...
    for (const rx of regexes) {
      assert.match(rx, /^[\p{L}\p{N}]+$/u, 'une regex ne doit contenir que du texte : ' + rx);
    }
  ...
  await t.test('le nombre de mots est borné', () => {
    ...
    const f = svc.filtreTexteRepli({}, Array.from({ length: 50 }, (_, i) => 'mot' + i).join(' '), esc);
